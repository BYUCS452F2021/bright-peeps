<template>
  <div>
    ...
    <div class="center_vertically">
      ...
      <img
        src="https://www.almanac.com/sites/default/files/styles/opengraph/public/image_nodes/chick-close-up-grass-crop.jpg?itok=ebheIGw2"
        alt="Avatar"
      />
  ...
  methods: {
    async signUp() {
      if (this.signUpUsername === '' || this.signUpPassword === '') {
        ...
        const response = await axios.post(
          'https://bright-peeps-api.azurewebsites.net/user',
          {
    ...
    async login() {
      if (this.loginUsername === '' || this.loginPassword === '') {
        ...
        const url =
          'https://bright-peeps-api.azurewebsites.net/user/username/' +
          this.loginUsername
        ...
        if (response.status === 200) {
          ...
          if (
            response.data.result.some(function (peep) {
            ...
            this.$store.state.username = lUsername
          } else {
            alert('Bad username or password. Please reenter and try again.')
            this.loginUsername = ''
            this.loginPassword = ''
